...
# Define la ruta del binario DENTRO del volumen persistente /app
PLAYIT_BIN="/app/playit"

# Si el binario de playit NO existe en el volumen...
if [ ! -f "$PLAYIT_BIN" ]; then
    echo "PlayIt binary not found, downloading (URL estática)..."
    
    # CRÍTICO: Usar la URL de descarga directa de PlayIt
